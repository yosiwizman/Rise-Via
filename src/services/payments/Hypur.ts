...
export class HypurProvider implements PaymentProvider {
  ...
  async createPaymentIntent(amount: number, customer: Customer, orderId: string): Promise<PaymentResult> {
    try {
      ...
      if (response.ok) {
        return {
          success: true,
          paymentIntentId: data.intent_id || `hypur_intent_${Date.now()}`,
  ...
  validateWebhook(payload: string | Buffer, signature: string): boolean {
    try {
      ...
      return !!(data && 
               typeof data.provider === 'string' && data.provider === 'hypur' && 
    ...
    try {
      ...
      switch (event.type) {
        case 'payment.completed':
          console.log('Hypur payment completed:', eventData.transaction_id);
          // Handle successful payment
          break;
        
        case 'payment.failed':
          console.log('Hypur payment failed:', eventData.transaction_id);
          // Handle failed payment
          break;
        
        case 'refund.processed':
          console.log('Hypur refund processed:', eventData.refund_id);
          // Handle refund completion
          break;
        
        default:
          console.log('Unhandled Hypur webhook event:', event.type);
      }
    } catch (error) {
      console.error('Failed to handle Hypur webhook:', error);
      throw error;
    }
  }
  ...
  async createCustomer(customer: Customer): Promise<string | null> {
    try {
      ...
      if (response.ok) {
        return data.customer_id || `hypur_customer_${Date.now()}`;
      } else {
        console.error('Failed to create Hypur customer:', data.error);
        return null;
      }
    } catch (error) {
      console.error('Failed to create Hypur customer:', error);
      return null;
    }
  }
  ...
  async getPaymentMethods(customerId: string): Promise<Stripe.PaymentMethod[]> {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        // Convert Hypur payment methods to Stripe-like format for compatibility
